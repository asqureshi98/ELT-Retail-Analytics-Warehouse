...
See also: [Architecture](architecture.md), [Data model](data_model.md), and [Project walkthrough](project_walkthrough.md).
...
- Metabase-to-PostgreSQL host: `postgres` (Docker Compose service hostname)
- Host-to-Metabase URL: `localhost:3000`

These credentials are development-only defaults and can be overridden through environment variables documented in [runbook.md](runbook.md).
...
2. Complete first-run setup with local development credentials when a setup token is present, or log in when Metabase is already configured.
3. Create or update the `Retail Warehouse` PostgreSQL database connection using Docker hostname `postgres`.
...
Dashboard visual layout is intentionally simple and API-driven. The script provisions deterministic grid positions for each card, but Metabase may adjust rendering details by image/API version. This workflow has been verified against `metabase/metabase:latest` resolving locally to Metabase `v0.61.3`.

## Portfolio Notes

The dashboards are designed to prove that the dbt marts can support business-facing analytics. They are not intended to replace a production BI design process with stakeholder review, row-level security, certified metrics, or environment promotion.
